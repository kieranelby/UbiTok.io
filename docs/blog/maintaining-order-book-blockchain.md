...
In our first UbiTok.io blog post, I'm going to write about the technique we've developed to efficiently maintain an order book entirely on the Ethereum blockchain, including performing best execution matching. Warning: this post is rather long and rather technical.
...
Since there's 10800 prices, we need 10800 bits. One nice thing about the Ethereum Virtual Machine is that it uses 256-bit words - which means that just 43 storage locations can hold all 10800 bits.
...
Of course, we still have to track the orders at each price. But luckily the Etherem Virtual Machine provides cheap unordered maps built-in - so we can have an unordered map from the price index (such as 5423) to a doubly linked list of the orders at that price.
